...
class CharacterIntegrationTests(unittest.TestCase):
    ...
    def test_builders_receive_complete_existing_dependencies(self):
        ...
        for builder in (
            ...
            build_combat_session,
        ):
            calls = [
                n for n in ast.walk(parsed)
                if isinstance(n, ast.Call)
                and isinstance(n.func, ast.Name)
                and n.func.id == builder.__name__
            ]
        ...
        for mode, section in [('quick', None), ('guided', 'creation')]:
            with self.subTest(mode=mode):
                ...
                open_new_character_dialog(
                    ...
                    create_rpg_character=create,
                    character_url=url,
                    notify_error=notify,
                )

                self.assertEqual(
                    set(ui.toggle.call_args.args[0].values()),
                    {'Création rapide', 'Création guidée'},
                )
                callback = next(
                    c.kwargs['on_click']
                    for c in ui.button.call_args_list
                    if c.args[0] == 'Créer'
                )
                callback()
    ...
    def test_tabs_and_selected_creation_route_and_quick_combat(self):
        ...
        panel = next(
            ...
            and n.name == 'rpg_character_panel'
        )
        calls = [n for n in ast.walk(panel) if isinstance(n, ast.Call)]
        tabs = [
            n.args[0].value
            for n in calls
            if ast.unparse(n.func) == 'ui.tab'
        ]
        self.assertEqual(
            tabs,
            [
        ...
        button = next(
            n for n in calls
            if ast.unparse(n.func) == 'ui.button'
            and n.args
            and isinstance(n.args[0], ast.Constant)
            and n.args[0].value == 'Combat rapide'
        )
        self.assertEqual(
            ast.unparse(
                next(
                    k.value
                    for k in button.keywords
                    if k.arg == 'on_click'
                )
            ),
            'open_combat_session',
        ...
        self.assertEqual(
            sum(
                ast.unparse(n.func) == '_create_character_dialog'
                for n in calls
            ),
            2,
        )
    ...
    def test_combat_reloads_latest_character_without_rebuilding_handle(self):
        callback = next(
            ...
            if isinstance(n, ast.FunctionDef)
            and n.name == 'open_combat_session'
        )
        ...
        env = dict(
            ...
            current_id=17,
            user_id=3,
            get_rpg_character=Mock(return_value=fresh),
            combat_session=handle,
            _safe_notify_error=Mock(),
        )
        exec(
            compile(
                ast.Module(body=[callback], type_ignores=[]),
                '<callback>',
                'exec',
            ),
            env,
    ...
    def test_modules_imported_without_back_reference(self):
        ...
        self.assertTrue(
            {'rpg_character_creation', 'rpg_combat_session'} <= modules
        )
    ...
    def test_jdr_version_and_release_note(self):
        ...
        note = next(
            n for n in app_versions.RELEASE_NOTES
            if n['app_key'] == 'rpg'
        )
